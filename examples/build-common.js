...
const commonArgs = `--display-chunks --display-max-modules 99999 --display-origins --display-entrypoints --output-public-path "dist/" ${extraArgs}`;
...
const doCompileAndReplace = (args, prefix, callback) => {
  ...
  if (fs.existsSync("dist"))
    for (const file of fs.readdirSync("dist")) fs.unlinkSync(`dist/${file}`);
  cp.exec(
    `${path.resolve(
      __dirname,
...
async.series(
  [
    callback =>
      doCompileAndReplace("--mode production", "production", callback),
    callback =>
      doCompileAndReplace(
    ...
    callback =>
      doCompileAndReplace("--mode none --output-pathinfo", "", callback)
  ],
  () => {
